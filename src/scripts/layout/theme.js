...
$(document).ready(function() {
  ...
  $(window).resize(function () {
	...
	*/
	...
	$('.card-stack-window').each(function(){
		var $this = $(this);
		var $stack = $this.find('.card-stack');
		var $cards = $stack.find('.card');
		var cardHeight = $this.outerHeight();
		var activeIndex = 0;
		// scroll spy the stack
		$stack.on('scroll', function(){
			$cards.each(function(n){
				let newPos = (cardHeight * n) - $stack.scrollTop();
				if( newPos <= 5 ){
					$(this).closest('.frame').addClass('pinned'); 
				}else{
					$(this).closest('.frame').removeClass('pinned'); 
				}
			});
			// update the nav active item
			let currentIndex = Math.floor($stack.scrollTop() / cardHeight);
			let $cardsNavItems = $this.find('.card-stack-nav li');
			if( currentIndex !== activeIndex ){
				$cardsNavItems.removeClass('active');
				$cardsNavItems.eq(currentIndex).addClass('active');
				activeIndex = currentIndex;
			}
		});

		// using the nav to navigate
		$this.find('.card-stack-nav a').on('click', function(e){
			e.preventDefault();
			$stack.animate({ scrollTop: $(this).data('index') * cardHeight })
		});
	});
